...
void GCanvasContext::InitFBO()
{
    ...
    {
        LOG_D("DefaultFboName:%s",DefaultFboName);
...
bool GCanvasContext::InitializeGLEnvironment()
{
    ...
    LOG_I("initializeGLEnvironment mWidth=%d,mHeight=%d ,glGetErrorCode=%d",mWidth,mHeight,eglGetError());
...
/**
 * @brief 计算模糊参数，确定是否需要递归调用
 * @param rect 模糊区域矩形
 * @param blur 模糊半径
 * @param scale 缩放因子
 * @param outBlurStep 输出：模糊步长
 * @param outScaleFactor 输出：缩放因子
 * @return 是否需要递归调用Blur
 */
bool GCanvasContext::PrepareBlurParams(const GRectf &rect, float blur, float scale,
                                        float &outBlurStep, float &outScaleFactor)
{
    const float Step = 5;
    outBlurStep = Step;
    outScaleFactor = 1 / Step;
    return blur * scale > Step;
}
 ...
 * @param rect 绘制区域矩形
 * @param draw 绘制回调函数
 * @param originFbo 输出的原始FBO对象
 * @param scale 缩放因子
 */
void GCanvasContext::DrawContentToFBO(const GRectf &rect, std::function<void()> draw,
                                       GFrameBufferObjectPtr &originFbo, float scale)
{
...
/**
 * @brief 执行水平模糊 passes
 * @param originFbo 原始FBO
 * @param rect 模糊区域矩形
 * @param blurStep 模糊步长
 * @param scaleFactor 缩放因子
 * @return 水平模糊后的FBO
 */
GFrameBufferObjectPtr GCanvasContext::PerformHorizontalBlur(GFrameBufferObjectPtr &originFbo,
                                                               const GRectf &rect,
                                                               float blurStep,
                                                               float scaleFactor)
{
...
/**
 * @brief 执行垂直模糊 passes，输出到屏幕或FBO
 * @param shadowFbo 水平模糊后的FBO
 * @param rect 模糊区域矩形
 * @param isOnScreen 是否输出到屏幕
 * @param outputFbo 输出FBO（如果isOnScreen为false）
 */
void GCanvasContext::PerformVerticalBlur(GFrameBufferObjectPtr &shadowFbo,
                                          const GRectf &rect, bool isOnScreen,
                                          GFrameBufferObjectPtr &outputFbo)
{
    if (isOnScreen)
    {
        // draw screen
...
//    float blurStep;
//    float scaleFactor;
//    bool needRecursion = PrepareBlurParams(rect, blur, scale, blurStep, scaleFactor);
//
//    if (needRecursion)
//    {
...
//    {
//        // draw origin
//        DrawContentToFBO(rect, draw, originFbo, scale);
//        blurStep = blur * scale;
//    }
//
//    Save();
...
//
//    // horizontal blur
//    auto shadowFbo = PerformHorizontalBlur(originFbo, rect, blurStep, scaleFactor);
//
//    // vertical blur
//    PerformVerticalBlur(shadowFbo, rect, isOnScreen, outputFbo);
//
//    Restore();
//}

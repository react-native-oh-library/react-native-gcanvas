...
protected:
    ...
    bool PrepareBlurParams(const GRectf &rect, float blur, float scale,
                           float &outBlurStep, float &outScaleFactor);
    void DrawContentToFBO(const GRectf &rect, std::function<void()> draw,
                          GFrameBufferObjectPtr &originFbo, float scale);
    GFrameBufferObjectPtr PerformHorizontalBlur(GFrameBufferObjectPtr &originFbo,
                                                 const GRectf &rect,
                                                 float blurStep,
                                                 float scaleFactor);
    void PerformVerticalBlur(GFrameBufferObjectPtr &shadowFbo,

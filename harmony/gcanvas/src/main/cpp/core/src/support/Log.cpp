...
GLogLevel g_log_level = LOG_LEVEL_DEBUG;

GLogLevel GetLogLevel() { return g_log_level; };

void SetLogLevel(GLogLevel logLevel) {
    if (g_log_level == logLevel)
        return;
#ifdef DEBUG
    GLogLevel oldLevel = g_log_level;
...
int TransLogLevel(GLogLevel logLevel) {
...
void LogExt(GLogLevel logLevel, const char *tag, const char *format, ...) {
    if (g_log_level > logLevel)
        return;
    va_list va;
    ...
    if (logLevel == LOG_LEVEL_INFO) {
        OH_LOG_Print(LOG_APP, LOG_INFO, LOG_DOMAIN, tag, "%{public}s", buffer);
    } else if (logLevel == LOG_LEVEL_DEBUG) {
        OH_LOG_Print(LOG_APP, LOG_DEBUG, LOG_DOMAIN, tag, "%{public}s", buffer);
    } else if (logLevel == LOG_LEVEL_WARN) {
        OH_LOG_Print(LOG_APP, LOG_WARN, LOG_DOMAIN, tag, "%{public}s", buffer);
    } else if (logLevel == LOG_LEVEL_ERROR) {
        OH_LOG_Print(LOG_APP, LOG_ERROR, LOG_DOMAIN, tag, "%{public}s", buffer);
    } else if (logLevel == LOG_LEVEL_FATAL) {
        OH_LOG_Print(LOG_APP, LOG_FATAL, LOG_DOMAIN, tag, "%{public}s", buffer);
    } else {
        OH_LOG_Print(LOG_APP, LOG_INFO, LOG_DOMAIN, tag, "%{public}s", buffer);
    }
#else

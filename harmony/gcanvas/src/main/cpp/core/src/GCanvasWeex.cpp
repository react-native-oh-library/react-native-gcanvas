...
#elifdef OHOS
#include "platform/ohos/GCanvas2DContextHarmony.h"
#else
#include <sys/time.h>
#endif
...
void GCanvasWeex::Render(const char *renderCommands, int length) {
    ...
    if (0 != mCanvasContext->mContextType) {
        ...
        glClear(GL_COLOR_BUFFER_BIT | GL_STENCIL_BUFFER_BIT);
        LOG_D("GCanvasWeex::Render:[2D] eglGetError code:%d", eglGetError());
...
#ifdef OHOS
    mCanvasContext->mCurrentState->mShader = ((GCanvas2DContextHarmony *) mCanvasContext)->GetShaderManager()->programForKey(
            "PATTERN");
#endif    
...
std::string GCanvasWeex::webglProc(int op, int sync, std::string args) {
    LOG_D("webglProc start.%s",args.c_str());
    if (args.c_str() == nullptr || args.length() == 0 ||
        !strcmp(args.c_str(), "")) {
        return "";
    }

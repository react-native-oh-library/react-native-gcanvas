...
#include <assert.h>
GFontManagerHarmony::GFontManagerHarmony(GCanvasContext *context) : GFontManager(context) {

}


GFontManagerHarmony::~GFontManagerHarmony() {
    mTreemap.Clear();
    mGlyphCache.ClearGlyphsTexture();
}


void GFontManagerHarmony::DrawText(const unsigned int *ucs,
...
float *GFontManagerHarmony::MeasureTextWidthHeight(const char *text, unsigned int textLength,
    ...
    LOG_D("MeasureTextWidthHeight text=%s,textLength=%d",text,textLength);
    ...
    if (fonts.size() > 0) {
        //文本高度本来应该去除上下空白的，但是laya1，还是有点问题，所以这个高度，还是用老的高度ascender+descender绝对�?       
...
float GFontManagerHarmony::MeasureText(const char *text,
...
float *GFontManagerHarmony::MeasureTextExt(const char *text, unsigned int textLength,
                                           gcanvas::GFontStyle *fontStyle) {
    float *tmpMeasure = MeasureTextWidthHeight(text, textLength, fontStyle);
    return tmpMeasure;
}

float *GFontManagerHarmony::PreMeasureTextHeight(const char *text,
...
void GFontManagerHarmony::AdjustTextPenPoint(std::vector<GFont *> fonts,
...
GFont *GFontManagerHarmony::GetFontByCharCode(wchar_t charCode, gcanvas::GFontStyle *fontStyle) {
    float devicePixelRatio = 1;
    if (mContext->GetHiQuality()) {
        devicePixelRatio = mContext->mDevicePixelRatio;
    }
    float size = fontStyle->GetSize() * devicePixelRatio;
     LOG_D("GetFontByCharCode devicePixelRatio:%f",devicePixelRatio);
...
GFont *GFontManagerHarmony::GetEmojiFont(wchar_t charCode, gcanvas::GFontStyle *fontStyle) {
    float devicePixelRatio = 1;
    if (mContext->GetHiQuality()) {
        devicePixelRatio = mContext->mDevicePixelRatio;
    }
...
void GFontManagerHarmony::FillTextInternal(GFont *font, bool isStroke, wchar_t charcode,
                                           float &x, float y) {
...
void GFontManagerHarmony::SetFontCache(GFontCache *fontCache) {

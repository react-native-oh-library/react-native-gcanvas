...
static PFNGLFRAMEBUFFERTEXTURE2DMULTISAMPLEIMGPROC getFramebufferTexture2DMultisampleEXT() {
    static PFNGLFRAMEBUFFERTEXTURE2DMULTISAMPLEIMGPROC proc = nullptr;
    static bool init = false;
    if (!init) {
        proc = (PFNGLFRAMEBUFFERTEXTURE2DMULTISAMPLEIMGPROC) eglGetProcAddress(
...
static PFNGLRENDERBUFFERSTORAGEMULTISAMPLEIMGPROC getRenderbufferStorageMultisampleEXT() {
    static PFNGLRENDERBUFFERSTORAGEMULTISAMPLEIMGPROC proc = nullptr;
    static bool init = false;
    if (!init) {
        proc = (PFNGLRENDERBUFFERSTORAGEMULTISAMPLEIMGPROC) eglGetProcAddress(
...
bool GFrameBufferObject::InitFBO(int width, int height, GColorRGBA color, bool enableMsaa, std::vector<GCanvasLog> *errVec)
{
    ...
    if (enableMsaa) {
        ...
        if (eglGetCurrentContext() == EGL_NO_CONTEXT) {
            // 可重试绑定或直接返回失败
            AppendErrorLogInfo(errVec, "no_egl_context", "...");
            return false;
        }
        // 默认samples为4，是否需要处理？
...
//                "glFramebufferTexture2DMultisampleEXT");
//        LOG_E("glFramebufferTexture2DMultisampleEXT start 1code:%d",eglGetError());
...
//        }
//        LOG_E("glFramebufferTexture2DMultisampleEXT end");
...
//            support_render_texture_msaa = false;
//        }
         glFramebufferTexture2DMultisampleEXT = getFramebufferTexture2DMultisampleEXT();
        if (!glFramebufferTexture2DMultisampleEXT) {
            support_render_texture_msaa = false;
        }
        // 同理获取另一个函数
        glRenderbufferStorageMultisampleEXT = getRenderbufferStorageMultisampleEXT();
        if (!glRenderbufferStorageMultisampleEXT) {
            support_render_texture_msaa = false;
        }
    ...
    LOG_W("initFBO> useMsaa=%d, support_render_texture_msaa=%d, glGetError=%d", useMsaa, support_render_texture_msaa, glGetError());
    ...
    if (useMsaa && glFramebufferTexture2DMultisampleEXT)
    {   
    ...
    if (useMsaa && openMsaaOk && glRenderbufferStorageMultisampleEXT)
    {
